...
var PingHub = exports.PingHub = Class.extend({
    ...
    httppost_pleaseNotify: function (req, res) {
        ...
        var param_names = ['port','path','protocol','url1'];
        ...
        var url_list = [];
        for (var i=1,an_url; an_url=req.body['url'+i]; i++) {
            url_list.push(an_url);
        }

        // Grab client address and optional domain param, need challenge if
        // they differ.
    ...
    pleaseNotify: function (cb, notify_procedure, port, path, protocol, 
        ...
        // Process each feed URL in parallel...
        async.forEach(url_list, function (feed_url, fe_next) {
            async.waterfall([
                ...
                // Get/create a unique registration for this request
                function (resp, wf_next) {
                    $this.notification_requests.getOrCreate(request, { 
                        success: function (obj) { 
                            // HACK: Fire-and-forget save to update modified timestamp.
                            obj.instance.save();
                            requests.push(obj.instance); 
                            wf_next(); 
                        },
    ...
    notifyOneViaXMLRPC: function (cb, nreq, challenge, http) {
        ...
        var req = client.request(options.method, options.path, { host: options.host });
    ...
    notifyOneViaHTTPPOST: function (cb, nreq, challenge, http) {
        ...
        var client = http.createClient(options.port, options.host);
        var req = client.request(options.method, 
            options.path + '?' + q_params, 
            { host: options.host });
        ...
        if ('POST' == options.method) { 
            // Seems like radio2 notify via POST wants the params in the URL,
            // not request body.
            req.write(q_params); 
        }
        req.end();
