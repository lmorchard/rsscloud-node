    ...
    pleaseNotify: function (cb, notify_procedure, port, path, protocol, 
        ...
        async.forEach(url_list, function (feed_url, fe_next) {
            async.waterfall([
                ...
                // Ensure there's one and only one registration - this can be an issue. (Why?)
                // TODO: Should the persistence layer instead use a primary key based on hash of attributes?
                function (resp_msg, existing, wf_next) {
                    if (existing.length) {
                        var current = existing.pop();
                        current.save({}, {
