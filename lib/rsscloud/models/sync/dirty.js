...
var DirtySync = exports.DirtySync = Class.extend({
    ...
    sync_getOrCreate: function (model, success, error, options) {
        var $this = this;
        var dirty = $this.bd.dirty;

        // HACK: This seems like a lot of work to get an URL
        var tmp = new model(options.data);
        tmp.collection = options.collection;
        tmp.id = tmp.hash();
        var url = _.isFunction(tmp.url) ? tmp.url() : tmp.url;

        var data = dirty.get(url);
        if (data) {
            success({ created: false, data: data });
        } else {
            var new_data = tmp.toJSON();
            dirty.set(url, new_data, function () { 
                success({ created: true, data: new_data });
            });
        }
    },
