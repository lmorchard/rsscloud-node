...
var LocmemSync = exports.LocmemSync = Class.extend({
    ...
    sync: function (method, model, success, error, options) {
        ...
        switch (method) {
            case 'create':
                if (!model.id) { 
                    data.id = model.id = model.attributes.id = model.hash();
                }
                if (!data.created) {
    ...
    sync_getOrCreate: function (model, success, error, options) {
        var $this = this;
        var id = model.prototype.hash(options.data);
        if (id in this.store) {
            success({ created: false, data: this.store[id] });
        } else {
            this.store[id] = options.data;
            success({ created: true,  data: options.data });
        }
    },
