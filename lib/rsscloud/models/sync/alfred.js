...
var AlfredSync = exports.AlfredSync = Class.extend({
    ...
    sync_fetchByFeedUrl: function (model, success, error, options) {
        ...
        .all(function (err, records) {
            if (records) for (var i=0,r; r=records[i]; i++) { 
                options.item(r.value); 
            }
            options.done();
        })
    },
    ...
    sync_fetchByClientAddrAndFeedUrl: function (model, success, error, options) {
        // NOTE: Tried using .stream() here, but nothing seems to happen when
        // there are no records available. Neither 'record' nor 'end' event fires.
        this.db.notifications.find({
