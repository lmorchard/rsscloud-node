...
// 
function XMLRPCDispatchHandler (handlers) {
    return function (req, res) {
        var params = req.body_XMLRPC.params,
            method = req.body_XMLRPC.method;
        var success_cb = function (rv) {
            res.send(new XMLRPCResponse([rv]).xml());
        };
        var fault_cb = function (code, message) {
            res.send(new XMLRPCFault(code, message).xml());
        };
        if (handlers.hasOwnProperty(method)) {
            try {
                ...
                handlers[method].apply(
                    handlers, 
                    [success_cb, fault_cb].concat(params)
                );
            } catch (e) {
                fault_cb(-32500, "Unexpected exception " + e);
            }
        } else {
            fault_cb(-32601, "requested method '"+method+"' not found");
        }
    }
}
